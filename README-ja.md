...
**重要**: `plugin/data.json`ファイルにはAPIキーが含まれており、`.gitignore`でGitから自動的に除外されています。

- **`plugin/data.json`を絶対にバージョン管理にコミットしないでください**
- このリポジトリをフォークまたはクローンした場合、`plugin/data.json`が`.gitignore`に含まれていることを必ず確認してください
- このファイルを誤ってコミットすると、APIキーが外部に漏洩します
